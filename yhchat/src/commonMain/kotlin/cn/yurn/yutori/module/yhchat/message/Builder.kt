...
class YhChatMessageBuilder(builder: MessageBuilder) : ExtendedMessageBuilder(builder) {
    ...
    inline fun html(block: HTML.() -> Unit) =
        HTML(yutori).apply(block).buildElement().apply { elements += this }
    ...
    @BuilderMarker
    class HTML(yutori: Yutori) : MessageBuilder(yutori), PropertiedMessageBuilder {
        override val properties = mutableMapOf<String, Any?>()
        override fun buildElement() = buildElement(HTML())
    }
}